...
function Band({
  ...
  // Calculate responsive anchor:
  // Desktop: align top-left under ABOUT
  // Mobile: shift right to hang over the '2' and raise up to prevent overlapping bio text
  const responsiveAnchor: [number, number, number] = isMobile
    ? [viewport.width * 0.15, viewport.height / 2 + 1.5, 0]
    : [-viewport.width / 2 + (viewport.width * 0.15), viewport.height / 2 - (viewport.height * 0.15), 0];
  ...
  const cardMap = useMemo(() => {
    ...
    const drawFitted = (img: any, rect: typeof FRONT_UV_RECT) => {
      ...
      try {
        ...
        console.error("Failed to draw fitted image:", e);
      }
    };

    if (frontImage && frontTex.image) drawFitted(frontTex.image, FRONT_UV_RECT);
    
    if (backImage && backTex.image && backImage !== null) {
      drawFitted(backTex.image, BACK_UV_RECT);
    } else {
      // Custom drawing for the back face: black background with white text "existing."
      ...
      ctx.fillStyle = '#ffffff';
      ctx.font = `${Math.round(rh * 0.06)}px "Space Grotesk", sans-serif`;
      ctx.textAlign = 'left';
      ctx.textBaseline = 'middle';
      ctx.fillText("existing.", rx + rw * 0.1, ry + rh * 0.5);
    }
  ...
  return (
    <>
      <group position={responsiveAnchor}>
